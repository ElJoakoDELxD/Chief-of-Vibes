...
| Identity guard. Refuses a git invocation that sets the commit author inline, so the agent observes its identity and never sets it (§7) | 2 | before every shell command | a visible `BLOCKED` message when it acts, nothing otherwise |
...
| Index check, in CI as `index-check`. Regenerates `INDEX.md` from the tree and compares, so a stale index fails rather than lying (§1) | 2 | on every pull request, whatever its base | a failed check on the pull request |
| Section check, in CI as `section-check`. Compares the core's map against the tree, so a pointer to a section cannot outlive it (§8) | 2 | on every pull request, whatever its base, and on demand with `tools/sections.sh` | a failed check naming each row and file that disagree |
| Prose gate, in CI as `prose-gate`. Scores every published file against the reference rate in §7 and names what is over it, which a reader weighs rather than a check deciding | 3 | on every pull request, whatever its base | the scores in the check log, and never a failure |
| PR guard, in CI as `pr-guard`. Rejects non-template files bound for `main`, and template changes with no version bump (§8) | 2 | on every pull request, whatever its base, each layer judged against that base (§8) | a failed check on the pull request, naming both rejections rather than the first |
