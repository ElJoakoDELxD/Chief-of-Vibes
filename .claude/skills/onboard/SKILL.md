...
5. **Budget.** Ask what inference the Principal funds, and how often. Say where the answer lands *before* asking for it: `memory/state.md` on the agent branch, which is public if this repository is. **A scale answers it completely.** *One plan*, *small and steady*, *what a side project bears*. A figure is the wrong answer in a public copy, and blank is a complete answer too. The triage plans against this field. It never reports against it, because the agent cannot see what a session cost (SYSTEM.md §8).
