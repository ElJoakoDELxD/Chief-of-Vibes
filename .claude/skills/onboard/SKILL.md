...
3. **Timezone.** Ask for the IANA zone (e.g. `America/Santiago`). Show one test timestamp and confirm it before persisting.
...
6. **Vault.** On the new branch, create:
   - `memory/state.md` — the SYSTEM.md §5 frontmatter, filled from the answers; `created` from `tools/now.sh`.
