...
- **The blueprint is `.blueprint`** on this branch, read from the file rather than assumed — every generation proposes there and none proxies through the copy it came from, and proposing into the wrong repository is not recoverable by editing the pull request.
