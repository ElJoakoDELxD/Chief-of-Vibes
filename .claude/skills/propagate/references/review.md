...
1. **Is the result real and stated, and of the kind the proposal claims?** Three kinds are admissible — a failure, a discovery validated from outside, and a judgment about the design. The third is approved on different questions: is it true of every copy, does acting on it change what anyone does tomorrow, is it a distinction rather than a preference. An unlabelled proposal resting on judgment is the one to send back — not because judgment is inadmissible, but because the reviewer was about to apply the wrong test. A proposal with nothing behind it is a preference. Internal evidence: ask for the tool and the output, and do not infer them generously because the writing is good — this system's own record is that the most convincing prose accompanied the most wrong conclusions. External evidence, including revenue, needs no internal blessing and is the stronger of the two; what it does need is an **attribution claim**, because a payment does not say which part earned it. Approve the mechanism the proposal names, not the outcome it points at — and if the record of the instance is missing (what it was, inside what system, doing what, and what the signal was), that is a blocking finding rather than a stylistic one: without it there is nothing for a reader in different conditions to judge, and the canon would be accepting a number on trust.
