...
The last field is the **model that served and its effort**, read from the session transcript and the environment. They are measured, never chosen, so the line shows which budget each reply actually spent. Written from memory, this field once said `high` while the environment measured `medium` (23-09-2026).
