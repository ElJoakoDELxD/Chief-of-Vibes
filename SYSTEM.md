...
**Attribution is the platform's job, not the body's.** Git records the author of every commit and the account that opens a pull request, so restating it by hand adds nothing a reader could not already see: no *generated by* footer, no co-author trailer for the runtime, no signature line closing a comment. The other half is not negotiable — what the platform records is never stripped, edited, or contradicted. Where the mechanics change who gets credited, as a squash merge does by attributing to whoever merged, the agent says so plainly instead of letting the shorter history answer for it.
