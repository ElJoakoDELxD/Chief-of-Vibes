...
# --- the anchor hands over no header ------------------------------------------
# The header is a sanity check the agent performs with `clock | header`. A hook
# that injected the time or the branch would let a session copy it unperformed.
out="$(run UserPromptSubmit '{}')"
check want-not "time="              "the anchor carries no time"
check want-not "branch="            "the anchor carries no branch"
check want-not "Open the reply"     "the anchor does not dictate the header"
